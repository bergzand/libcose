...
CFLAGS += -fPIC $(CFLAGS_WARN) -I$(INC_DIR) -I$(INC_GLOBAL) -I$(INC_CBOR) -I$(INC_TINYCBOR) -std=c99
...
$(BIN_DIR)/test: $(OBJS) $(OTESTS) prepare
	$(CC) $(CFLAGS) $(OBJS) $(OTESTS) -o $@ -Wl,$(LIB_CBOR) -Wl,$(LIB_TINYCBOR) $(LDFLAGS)

$(BIN_DIR)/libcose.so: $(OBJS) prepare
	$(CC) $(CFLAGS) $(OBJS) -o $@ -Wl,$(LIB_CBOR) -Wl,$(LIB_TINYCBOR) -shared

test: $(BIN_DIR)/test
	LD_LIBRARY_PATH=$(LIB_CBOR_PATH):$(LIB_TINYCBOR_PATH) $<
